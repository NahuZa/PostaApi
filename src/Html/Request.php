 ...
 * 
 * @apiSuccess {Number} id The ID of the newly created county.
 * @apiSuccessExample Success-Response:
 *      HTTP/1.1 201 Created
 *      {
 *          "id": 5
 *      }
 * @apiError BadRequest Missing or invalid data.
 */
    private static function postRequest()
    {
        ...
        switch ($resource) {
            ...
            case 'cities':
                $data = self::getRequestData();
                if (isset($data['city'], $data['zip_code'], $data['id_county'])) {
                    $repository = new CityRepository();
                    $newId = $repository->create($data);
                    $code = 201;
                } else {
                    $code = 400; 
                    Response::response([], $code, "Invalid data");
                    return;
                }
                Response::response(['id' => $newId], $code);
                break;
 ...
 * @apiVersion 1.0.0
 * 
 * @apiParam {String} name New name for the county.
 * 
 * @apiSuccessExample Success-Response:
 *      HTTP/1.1 202 Accepted
 * @apiError NotFound The county does not exist.
 */
    private static function putRequest()
    {
        ...
        switch ($resource) {
            ...
            case 'cities':
                ...
                $entity = $repository->find($id);
                $code = 404;
                if ($entity && isset($putRequestData['city'], $putRequestData['zip_code'], $putRequestData['id_county'])) {
                    $result = $repository->update($id, [
                        'city' => $putRequestData['city'],
                        'zip_code' => $putRequestData['zip_code'],
                        'id_county' => $putRequestData['id_county']
                    ]);
                    if ($result) {
                        $code = 202;
                    }
